...
async def async_setup_entry(hass: HomeAssistant, config_entry, async_add_entities) -> None:
    ...
    # DO NOT EXPOSE KEY IN LOGS!! _LOGGER.debug(f"Config entry data: {config_entry.data}")
    ...
    entities = [
        NEOWatcherRateLimitSensor(coordinator, "X-RateLimit-Limit", "Your NASA RateLimit",
                                  SensorEntityDescription(key="rate_limit_limit")),
        NEOWatcherRateLimitSensor(coordinator, "X-RateLimit-Remaining", "Your NASA RateLimit remaining calls",
                                  SensorEntityDescription(key="rate_limit_remaining")),
        NEOWatcherTotalPHObjectsSensor(coordinator),  # Renamed sensor
        NEOWatcherTotalObjectsSensor(coordinator)  # New sensor
    ]
...
class NEOWatcherFeedSensor(CoordinatorEntity, SensorEntity):
    ...
    async def _async_fetch_and_set_extra_attributes(self):
        ...
        horizon_url = f"https://ssd.jpl.nasa.gov/api/horizons.api?format=json&COMMAND=%27DES%3D{name_urlencoded}%27"
        ...
        self._extra_attributes = {
            ...
            "estimated_diameter_min_km": data.get("estimated_diameter", {}).get("kilometers", {}).get(
                "estimated_diameter_min"),
            "estimated_diameter_max_km": data.get("estimated_diameter", {}).get("kilometers", {}).get(
                "estimated_diameter_max"),
            "estimated_diameter_min_mi": data.get("estimated_diameter", {}).get("miles", {}).get(
                "estimated_diameter_min"),
            "estimated_diameter_max_mi": data.get("estimated_diameter", {}).get("miles", {}).get(
                "estimated_diameter_max"),
...
class NEOWatcherRateLimitSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator: NeoWatcherCoordinator, header_name: str, name: str,
        ...
        self._attr_unique_id = f"neo_watcher_{name.lower().replace('-', '_')}"
        self._attr_attribution = ATTRIBUTION
...
class NEOWatcherTotalPHObjectsSensor(CoordinatorEntity, SensorEntity):
    """Representation of a NEO Watcher total potentially hazardous objects sensor."""

    def __init__(self, coordinator: NeoWatcherCoordinator) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._attr_name = "Neo Watcher Total Potentially Hazardous Objects"
        self._attr_unique_id = "neo_watcher_total_potentially_hazardous_objects"  # Updated unique ID
        self._attr_attribution = ATTRIBUTION
        self._attr_native_value = len(self.coordinator.data)
        _LOGGER.debug("NEOWatcherTotalPHObjectsSensor initialized.")
...
class NEOWatcherTotalObjectsSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator: NeoWatcherCoordinator) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._attr_name = "Neo Watcher Total Objects"
        self._attr_unique_id = "neo_watcher_total_objects"
        self._attr_attribution = ATTRIBUTION
        self._attr_native_value = len(self.coordinator.near_earth_objects)
        _LOGGER.debug("NEOWatcherTotalObjectsSensor initialized.")

    @property
    def native_value(self) -> int:
        """Return the state of the sensor."""
        return len(self.coordinator.near_earth_objects)
