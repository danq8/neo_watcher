...
async def async_setup_entry(hass: HomeAssistant, config_entry, async_add_entities) -> None:
    ...
    async_add_entities([NEOWatcherStatsSensor(coordinator)])

    # Loop around to get the top 6 hazardous objects
    _LOGGER.debug(f"Found {len(coordinator.all_hazardous_objects)} potentially hazardous objects and {len(coordinator.all_objects)} total objects.")
    hazardous_feed_entities: List[Entity] = []
    for i in range(min(6, len(coordinator.all_hazardous_objects))):
        _LOGGER.debug(f"Creating NEOWatcherFeedSensor for hazardous object at index {i}.")
        hazardous_feed_entities.append(NEOWatcherFeedSensor(hass, coordinator, i, i+1))
    _LOGGER.debug(f"Adding {len(hazardous_feed_entities)} hazardous feed entities to Home Assistant.")
    
    # Loop around to get the top 6 non hazardous objects
    _LOGGER.debug(f"Found {len(coordinator.all_non_hazardous_objects)} non hazardous objects and {len(coordinator.all_objects)} total objects.")
    non_hazardous_feed_entities: List[Entity] = []
    for i in range(min(6, len(coordinator.all_non_hazardous_objects))):
        _LOGGER.debug(f"Creating NEOWatcherNonHazardousFeedSensor for non hazardous object at index {i}.")
        non_hazardous_feed_entities.append(NEOWatcherNonHazardousFeedSensor(hass, coordinator, i, i+1))
    _LOGGER.debug(f"Adding {len(non_hazardous_feed_entities)} non hazardous feed entities to Home Assistant.")
    ...
    await coordinator.async_fetch_horizon_data_for_objects(hass, hazardous_feed_entities + non_hazardous_feed_entities)
...
class NEOWatcherStatsSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator: NeoWatcherCoordinator) -> None:
        ...
        _LOGGER.debug("NEOWatcherStatsSensor initialized.")
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return the state attributes."""
        return {
            "Weeks ahead": self._weeks_ahead,
            "From date": self._from_date,
            "To date": self._to_date,
            "Total Near Earth Objects found": self._total_objects,
            "Total potentially hazardous Near Earth Ojects found": self._total_potentially_hazardous_objects,
            "Your NASA API rate limit": self._rate_limit_limit,
            "Your NASA API Rate limit remaining": self._rate_limit_remaining,
            "Process Start Time": self._process_start_time,
